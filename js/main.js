...
if (faqsSection) {
  const dynamicContentWrapper = faqsSection.querySelector('#faq-dynamic-content');

  if (dynamicContentWrapper) {
    // 1. Inject Accordion Items directly into #faq-dynamic-content
    dynamicContentWrapper.innerHTML = `
    ...
    // 2. Dynamic Background Setup & Gradient Switcher
    const bgSources = [
    ...
    // 3. Accordion Toggle Listener (Unified Single-Open Behavior)
    const accordionItems = dynamicContentWrapper.querySelectorAll('.accordion-item');
